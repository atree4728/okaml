...
let generalize tyenv ty = TySchema (free_tyvars tyenv ty, ty)
...
and infer_let is_rec tyenv decls cont =
  ...
  let* inferred_triple =
    ...
    |> map_m (fun (name, expr) ->
      ...
      then
        ...
        let* (TySchema (_, ty_placeholder)) = Env.lookup name tyenv' in
  ...
  let* subst = unify constr_bindings in
  (* weak type variables *)
  (* the target of unifing is tyenv, not tyenv', because tyenv' necessarily knows binding targets so they cannot be polymorphic. *)
  let unified_tyenv = Env.map (ty_schema_subst subst) tyenv in
  let new_tyenv =
    inferred_triple
    |> List.map (fun (name, ty, _) ->
      let unified_ty = ty_subst subst ty in
      let abs_tyvars = free_tyvars unified_tyenv unified_ty in
      name, TySchema (abs_tyvars, unified_ty))
    |> Env.of_list
...
let infer_letcmd is_rec tyenv decls =
  ...
  |> Myresult.map_m (fun (name, expr) ->
    ...
    Ok (name, ty_subst subst type_cont))
  |> Myresult.map (fun ty_bindings ->
...
let infer_cmd tyenv command =
  ...
  | CExp expr ->
    ...
    let* subst = unify constraints in
    let unified = ty_subst subst type_expr in
    Ok ([ unified ], tyenv)
